...
# 1. [구조 유지] 페이지 설정 및 제목
st.set_page_config(page_title="유기농 통합 가격 관리 시스템 v3.6", layout="wide")
st.title("🥬 홍성유기농-유기농부 가격 협업 플랫폼 v3.6")
...
# 6. [구조 유지] 하이브리드 계산 엔진 (원본 수식 100% 보존)
    ...
    for i in range(len(temp_df)):
        try:
            ...
            if is_rev:
                ...
                status_icon, name_prefix = "🟠", f"🔄 {name}"
            else:
                if tgt_mode == "판매가 기준":
...
# 7. [교체] 실시간 자동 계산 에디터 (대표님 요청 사항 반영)
st.info(f"💡 현재 **[{user_role}]** 권한 | 입력 후 Enter를 치면 마진이 즉시 자동 계산됩니다.")
...
edited_df = st.data_editor(
    ...
    hide_index=True,
    key="pricing_lab_editor" # [교체] 입력값 0 리셋 방지를 위한 고유 키
)

# [교체 핵심] 변경 감지 즉시 자동 계산 실행
...
# 9. [구조 유지] 하단 상태 정보 표기
st.sidebar.markdown("---")
st.sidebar.caption(f"Pricing Lab v3.6 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")