...
# 4. [구조 유지] 데이터 로드 함수 (데이터 형식 무결성 확보)
...
def load_data():
    try:
        ...
        # 2. 숫자형 컬럼 강제 변환 (StreamlitAPIException 방지)
...
edited_df = st.data_editor(
    ...
    column_config={
        ...
        "마진액(원)": st.column_config.NumberColumn("마진액(원)", disabled=True),
        "목표대비(+/-)": st.column_config.NumberColumn("목표대비(+/-)", disabled=True),
        "수수료율(%)": st.column_config.NumberColumn("수수료율(%)", format="%.1f%%"),
        "수수료액(원)": st.column_config.NumberColumn("수수료액(원)", disabled=True),
        "판매가(원)": st.column_config.NumberColumn("판매가(원)"),
